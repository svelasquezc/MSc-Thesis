...
%El ``químico'' como concepto comprende un conjunto de ``transferencias'' en las que se describen las cinéticas en no-equilibrio entre dos fluidos que pueden acarrear tal químico. Una transferencia es direccional, por lo que existe una cantidad de químico (``cantidad transferida'') que se transfiere de un ``fluido de origen'' a un ``fluido de destino'' y la ``cantidad transferida'' se calcula por medio de una ``función''. La ``presencia'' de éste en un fluido implica que se generen ``afectaciones'' de las propiedades de tal ``fluido de acarreo'', esta afectación se aplica por medio de una función que inserta el ``ingeniero de estimulación'' al preparar el químico.
...
Es importante notar también, que la función ``ubicar'' se encarga de calcular la posición correspondiente en el vector residual que agrupa todas las incógnitas a resolver. El proceso de cálculo de residuales se muestra en \ref{fig:Residual}. \\ % Residual
