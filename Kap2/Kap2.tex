...
En esta Tesis de Maestría, se modela el químico como \cite{ValenciaJD2016}, \cite{MozoID2017} e \cite{IsazaCN2017} proponen. En estos trabajos, un componente, especie o químico se transporta dentro de los fluidos existentes en el yacimiento y se puede transferir entre ellos. Adicionalmente, consideran que existen posibles modificaciones de las propiedades del fluido que acarrea tal químico. Las ecuaciones de conservación de volumen para el químico, que se acoplan al BOM, son las siguientes:

\begin{align}
\label{ec:quimico}
&\frac{\partial}{\partial t} \left[w_{k,f} \left( \frac{\phi S_{f}}{B_{f}} \right) \right] - \nabla \cdot \left( \frac{w_{k,f}}{B_{f}} \vec{u_{f}} \right) - w_{k,f}\tilde{q}_{f} - \sum_{f'\neq f}\dot{m}_{k,f \rightarrow f'}= 0 
\end{align}{}

La Ecuación \ref{ec:quimico} es una generalización en la que se presenta un químico $k$ que se transporta en un fluido $f$, y se puede transferir a uno o múltiples fluidos $f'$. Más aún, se considera que existen fuentes y sumideros de tal químico. Cabe notar que, $w_{k,f}$ es una fracción volumétrica del químico en el fluido.\\

Por otro lado, en el químico se involucran ecuaciones constitutivas adicionales en las que se relacionan las transferencias de un químico entre los fluidos existentes y las modificaciones a las propiedades de los fluidos en los que éste exista, estas ecuaciones constitutivas se presentan para las transferencias como:

\begin{align}
	\label{ec:transferencias}
	&\dot{m}_{k,f \rightarrow f'} = f(w_{k,f})
\end{align}{}

Y para las modificaciones al flujo se consideran específicamente la viscosidad y la permeabilidad relativa del fluido así:

\begin{align}
\label{ec:alteracionkr}
&k_{rf} = k_{rf, inicial} + f(w_{k,f})\\
\label{ec:alteracionu}
&\mu_{f} = \mu_{f, inicial} + f(w_{k,f})
\end{align}{}

La discretización de la Ecuación \ref{ec:quimico}, en cada uno de los fluidos en que un químico esté presente, es análoga a la Ecuación \ref{ec:aguaDiscretizacion}. Las funciones de transferencias y de modificaciones, en las Ecuaciones \ref{ec:transferencias}, \ref{ec:alteracionkr} y \ref{ec:alteracionu}, sólo multiplican el volumen de la celda en su discretización.\\ 

Los supuestos para este modelo son los siguientes:
\begin{itemize}
	\item Las funciones de transferencia y modificaciones se pueden expresar como función única de la concentración del químico en el fluido $f$, y son términos correctivos.
	\item Se desprecia la difusión y dispersión de los químicos.
	\item No existen aportes de volumen al fluido $f$ que acarrea el químico $k$.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Línea (Concepto-Nota)}: Sirve para conectar un concepto a una nota o instancia \citep{zapata2007phd,zapata2012unc}.
